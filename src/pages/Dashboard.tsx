...
export default function Dashboard({ prospects, onOpenAdd }: DashboardProps) {
  ...
  const funnelData = [
    { label: 'Prospects', val: stats.total, className: 'bg-status-prospect' },
    { label: 'Contactés', val: stats.contacted, className: 'bg-status-contacted' },
    { label: 'Démos', val: stats.demos, className: 'bg-status-demo' },
    { label: 'Signés', val: stats.signed, className: 'bg-status-signed' },
  ];
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        ...
        <StatCard label="Contactés" value={stats.contacted} icon={MessageSquare} />
        <StatCard label="Démos" value={stats.demos} icon={ExternalLink} />
        <StatCard label="Signés" value={stats.signed} icon={Briefcase} />
        <StatCard label="Refus" value={stats.refused} icon={X} />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-card p-6 rounded-xl border border-border shadow-sm">
          <h3 className="font-bold text-foreground mb-6 flex items-center gap-2">
            <TrendingUp size={18} className="text-primary" /> Tunnel de Conversion
          </h3>
          ...
          <div className="space-y-4">
            ...
            {prospects.length === 0 && (
              <div className="text-center py-8">
                ...
                <button onClick={onOpenAdd} className="text-sm text-primary font-semibold mt-2 hover:underline">Ajouter un prospect</button>
              </div>
            )}
          </div>
        </div>
